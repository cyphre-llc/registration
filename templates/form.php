...
<?php foreach ( $_['errormsgs'] as $errormsg ) {
	echo "<p>$errormsg</p>";
		...
		<input type="text" name="user" id="user" required="" value="<?php echo $_['entered_data']['user']; ?>" />
		...
		<p class="infield groupmiddle">
			...
			<input type="password" name="password" id="password" value="" data-typetoggle="#show" placeholder="" required="" original-title="" style="display: inline-block;">
		...
		<p class="infield groupbottom">
			<label for="password-clone"><?php print_unescaped($l->t( 'Password retype' )); ?></label>
			<input type="password" name="password-clone" id="password-clone" autocomplete="off" original-title="">
			<img style="top:2.7em;" id="password-icon" class="svg" src="<?php print_unescaped(image_path('', 'actions/password.svg')); ?>" alt=""/>
		</p>
